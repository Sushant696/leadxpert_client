...
import { Check, Minus, Zap, Building2, Rocket, ArrowRight, HelpCircle } from "lucide-react"
...
const faqs = [
  ...
  },
  {
    ...
    answer: "Every paid plan comes with a 14-day free trial. No credit card required. Cancel anytime.",
  },
  {
  ...
  },
  {
    ...
    answer: "Yes! We offer 50% off for verified nonprofit organizations and educational institutions. Contact our team to apply.",
  },
  {
    question: "What happens to my data if I cancel?",
    answer: "Your data is yours. You can export everything before canceling, and we keep your data for 30 days after cancellation in case you change your mind.",
  },
  {
    question: "Is my data secure?",
    answer: "Absolutely. We use industry-standard AES-256 encryption at rest and TLS 1.3 in transit. We are SOC 2 Type II compliant.",
  },
]
            ...
            return (
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <p className={cn("text-sm", isPro ? "text-primary-foreground/80" : "text-muted-foreground")}>
                    {plan.description}
                  </p>
                </div>
                ...
                <div className="mb-8">
                  <div className="flex items-end gap-1">
                    ...
                    {price > 0 && (
                      <span className={cn("text-sm mb-2", isPro ? "text-primary-foreground/70" : "text-muted-foreground")}>
                        /user/mo
                      </span>
                    )}
                  </div>
                  {isYearly && price > 0 && (
                    <p className={cn("text-xs mt-1", isPro ? "text-primary-foreground/70" : "text-muted-foreground")}>
                ...
                <ul className="space-y-3 flex-1">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-center gap-3">
                      {feature.included ? (
                        <Check className={cn("w-4 h-4 shrink-0", isPro ? "text-primary-foreground" : "text-green-500")} />
                      ) : (
                        <Minus className={cn("w-4 h-4 shrink-0", isPro ? "text-primary-foreground/40" : "text-muted-foreground/40")} />
                      )}
                      <span className={cn(
                        "text-sm",
                        !feature.included && (isPro ? "text-primary-foreground/50" : "text-muted-foreground/50")
                      )}>
                        {feature.text}
                      </span>
                    </li>
      ...
      <section className="py-12 px-4 border-y border-border bg-muted/30">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8 opacity-50">
            {["TechVentures", "Growth Partners", "StartupHub", "SalesForce Pro", "DevAgency"].map((company) => (
              <span key={company} className="text-lg font-bold text-foreground tracking-tight">
                {company}
              </span>
            ))}
          </div>
